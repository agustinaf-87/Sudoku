...
//check if duplicate number in colum
...
//check if duplicate number in sudoku box
...
//check in row, col and sudoku box
const isSafe = (grid, row, col, value) => {
    return isColSafe(grid, col, value) && isRowSafe(grid, row, value) && isBoxSafe(grid, row - row%3, col - col%3, value)
...
const shuffleArr = (arr)=>{
    ...
    while(curr_index !== 0){
        ...
        let temp_arr = arr[curr_index];
        arr[curr_index] = arr[rand_index];
        arr[rand_index] = temp_arr;
    }
    return arr; 
}
...
const sudokuCreate =(grid)=>{
    ...
    let number_list = shuffleArr([...CONSTANT.NUMBERS]);
